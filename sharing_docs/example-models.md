...
仓库里用 [vulkantorch / VulkanTorch.Net](../README.md) 移植了三个小模型（两个 TTS、一个 OCR）。
...
HiggsTTS（4B）两个库各移植了一份：Python 在 [`example/python/higgstts_py/`](../example/python/higgstts_py/)，
...
（[`example/python/ocr_py/`](../example/python/ocr_py/)），自包含 `.pyd`。输入裁好的单行
