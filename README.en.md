...
Four modes: `synth` (default, ref audio + text → wav), `encode` (only `encode_ref`), `decode` (RVQ codes → wav) and `serve` (streaming HTTP endpoint, see [sharing_docs/higgstts.en.md](sharing_docs/higgstts.en.md)). Args map 1:1 to the Python version, plus `--no-graph-cache`; `-h` / `--help` prints them all.
