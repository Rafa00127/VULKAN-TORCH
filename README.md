...
四个模式：`synth`（默认，参考音频+文本→wav）、`encode`（只跑 `encode_ref`）、`decode`（RVQ codes→wav）、`serve`（流式 HTTP 接口，见 [sharing_docs/higgstts.md](sharing_docs/higgstts.md)）。参数与 Python 版一一对应，另有 `--no-graph-cache`；`-h` / `--help` 打印全部参数。
